...
public class PresenceRepository {
  ...
  private static final String GET_PRESENCE_BY_CHILD_AND_DATE = "" +
    "select date, state, child, absence_reason, author from presence " +
    "where child = ? and date = ?";
  private static final String CREATE_PRESENCE = "" +
    "insert into presence (date, state, child, absence_reason, author)" +
    " values (?, ?, ?, ?, ?);";
  private static final String GET_PRESENCE_BY_CHILD = "" +
    "select date, state, child, absence_reason, author " +
    " from presence where child = ?";
  ...
  public List<Presence> getPresenceByChild(Child child){
    log.debug("getPresenceByChild " + child.toString());
    return jdbcTemplate.query(GET_PRESENCE_BY_CHILD, new Object[] {child.getId()}, presenceRowMapper());
  }
